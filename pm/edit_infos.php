		...
		if($_POST['password']==$_POST['passverif'])
		{
			...
			if(strlen($_POST['password'])>=6)
			{
				...
				if(preg_match('#^(([a-z0-9!\#$%&\\\'*+/=?^_`{|}~-]+\.?)*[a-z0-9!\#$%&\\\'*+/=?^_`{|}~-]+)@(([a-z0-9-_]+\.?)*[a-z0-9-_]+)\.[a-z]{2,}$#i',$_POST['email']))
				{
					...
					$dn = mysql_fetch_array(mysql_query('select count(*) as nb from members where username="'.$username.'"'));
					//We check if the username changed and if it is available
					if($dn['nb']==0 or $_POST['username']==$_SESSION['username'])
					{
						//We edit the user informations
						if(mysql_query('update members set username="'.$username.'", password="'.$password.'", email="'.$email.'" where id="'.mysql_real_escape_string($_SESSION['userid']).'"'))
						{
...
<?php
	...
	if($form)
	{
		...
		else
		{
			...
			$dnn = mysql_fetch_array(mysql_query('select username,password,email from members where username="'.$_SESSION['username'].'"'));
